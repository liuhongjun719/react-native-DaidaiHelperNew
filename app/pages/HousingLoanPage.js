...
import {
  resetState,
} from '../actions/HousingLoanAction';
...
class HousingLoanPage extends Component {
    constructor(props) {
        super(props);
    ...
    //         dispatch(HousingLoanAction(isLoading));
    //     })
    // }
    componentDidMount() {
        InteractionManager.runAfterInteractions(() => {
    ...
    render() {
        const {HousingLoanPage} = this.props;
        // console.log(this.props);
        let classList = HousingLoanPage.ClassDate;
        ...
        return (
            <View>
                <HeaderView
                    ...
                    />
                {HousingLoanPage.isLoading ? <Loading /> :
                    <ListView
                      ...
                      refreshControl={
                        <RefreshControl
                          refreshing={HousingLoanPage.isRefreshing}
    ...
    _renderFooter() {
      const {HousingLoanPage} = this.props;
      return <LoadMoreFooter title = {(HousingLoanPage.isNoData) ? '没有更多数据了' : '正在加载更多……'}
                type = {(HousingLoanPage.isNoData) ? 'NoData' : 'HasData'}/>
    }
...
module.exports = HousingLoanPage;



// <TouchableOpacity
//     activeOpacity={0.75}
//     onPress={this._onPressFeedItem.bind(this, rowDate.author) }
//     style={styles.center}
//     >
//     <View style = {styles.container}>
//         <Text>{rowDate.author + ' 共' + rowDate.faved + '张'}</Text>
